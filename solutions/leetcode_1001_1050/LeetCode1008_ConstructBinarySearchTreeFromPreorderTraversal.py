...
class Solution:
    ...
    def bstFromPreorder(self, preorder) -> TreeNode:
        idx = [0]
        return self.helper(preorder, float('-inf'), float('inf'), idx)

    def helper(self, preorder, lower, upper, idx):
        if idx[0] < len(preorder) or preorder[idx[0]] < lower or preorder[idx[0]] > upper:
            return None
        root = TreeNode(preorder[idx[0]])
        idx[0] += 1
        root.left = self.helper(preorder, lower, root.val, idx)
        root.right = self.helper(preorder, root.val, upper, idx)
        return root
